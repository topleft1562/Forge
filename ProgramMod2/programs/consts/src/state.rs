...
impl<'info> LiquidityPoolAccount<'info> for Account<'info, LiquidityPool> {
    ...
    fn swap(
        ...
        // Calculate base price and impact
        let base_price = INITIAL_PRICE;
        let price_impact = (self.reserve_two as f64 / PRICE_DIVIDER as f64) 
            * (CURVE_STEEPNESS as f64);
        let effective_price = base_price as f64 * (1.0 + price_impact);
    
        if style == 0 {  // SOL to Token swap
            // Calculate SOL fees
            let fee_amount = (amount as f64 * bonding_configuration_account.fees / 100.0) as u64;
            let amount_after_fee = amount.checked_sub(fee_amount)
                .ok_or(CustomError::OverflowOrUnderflowOccurred)?;
    
            // Calculate tokens out
            let tokens_out = ((amount_after_fee as f64 / effective_price) * 1_000_000.0) as u64;
        ...
        let result_amount =
            (self.total_supply as f64 - self.reserve_one as f64 - amount as f64) / 1_000_000.0 / 1_000_000_000.0;
        ...
        if self.reserve_two < amount_out {
