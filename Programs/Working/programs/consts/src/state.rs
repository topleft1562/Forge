...
// Apply SOL fee (0.1%)
    ...
    amount_out = (amount_after_fee as f64 / final_price).floor() as u64;
    amount_out = amount_out.min(self.reserve_one);
    ...
    let price_adjustment = ((tokens_at_current_price as u128 / 2 / PRICE_INCREMENT_STEP) as f64) * PRICE_INCREMENT;
    final_price = (current_price - price_adjustment) * SELL_REDUCTION;

    // Compute SOL received using adjusted price
    let sol_out = (amount as f64 * final_price).floor() as u64;
